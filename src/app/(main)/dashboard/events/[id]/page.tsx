...
export default function AdminEventPage() {
  ...
  const openEditModal = async (reg: Registration) => {
    ...
    setEditState({
      ...
      volunteeringCultural: Boolean(reg.volunteeringCultural),
      needsRide: Boolean(reg.needsRide),
      ridePickupAddress: reg.ridePickupAddress ?? '',
  ...
  const saveEditRegistration = async () => {
    ...
    const res = await fetch(`/api/admin/events/${eventId}`, {
      ...
      body: JSON.stringify({
        ...
        volunteeringCultural: editState.volunteeringCultural,
        needsRide: editState.needsRide,
        ridePickupAddress: editState.needsRide ? editState.ridePickupAddress : null,
        donationAmount: editState.donationAmount,
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  {['Member', 'Volunteer', 'Details', 'Attendees', 'Payment', 'Registration', 'Actions'].map((h) => (
                  ...
                  <tr key={r.id}>
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-1 text-xs">
                        {r.volunteeringCultural && <span className="block text-purple-700 font-medium">+ Cultural Volunteer</span>}
                        {r.needsRide && (
                          <div>
                            <span className="block text-blue-700 font-medium">Needs Ride</span>
                            {r.ridePickupAddress && <span className="block text-slate-500">{r.ridePickupAddress}</span>}
                          </div>
                        )}
                        {!r.photoConsent && <span className="block text-amber-700 font-medium">No Photo Consent</span>}
                        {Number(r.donationAmount) > 0 && <span className="block text-green-700 font-medium">Donation: ${r.donationAmount}</span>}
                        {r.notes && <span className="block text-slate-500 italic">{r.notes}</span>}
                        {!r.volunteeringCultural && !r.needsRide && r.photoConsent && !Number(r.donationAmount) && !r.notes && (
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-5">
              ...
              <label className="flex items-center gap-3 text-sm text-slate-700 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded border-slate-300 text-blue-600"
                />
                <span className="font-medium">General Volunteering</span>
              </label>
              ...
              <label className="flex items-center gap-3 text-sm text-slate-700 cursor-pointer">
                <input
                  type="checkbox"
                  checked={editState.volunteeringCultural}
                  onChange={(e) => setEditState((prev) => prev ? { ...prev, volunteeringCultural: e.target.checked } : null)}
                  className="w-4 h-4 rounded border-slate-300 text-blue-600"
                />
                <span className="font-medium">Cultural Volunteering</span>
              </label>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-3 text-sm text-slate-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={editState.needsRide}
                    onChange={(e) => setEditState((prev) => prev ? { ...prev, needsRide: e.target.checked } : null)}
                    className="w-4 h-4 rounded border-slate-300 text-blue-600"
                  />
                  <span className="font-medium">Needs Ride Pickup</span>
                </label>
                {editState.needsRide && (
                  <input
                    type="text"
                    placeholder="Pickup address"
                    value={editState.ridePickupAddress}
                    onChange={(e) => setEditState((prev) => prev ? { ...prev, ridePickupAddress: e.target.value } : null)}
                    className="w-full px-3 py-2 text-sm border border-slate-300 rounded-md placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                )}
              </div>
              ...
              {/* Donation Amount */}
              <div>
                <p className="text-sm font-medium text-slate-700 mb-1">Donation Amount ($)</p>
                <input
                  type="number"
                  min={0}
                  placeholder="0"
                  value={editState.donationAmount || ''}
                  onChange={(e) => setEditState((prev) => prev ? { ...prev, donationAmount: Number(e.target.value) || 0 } : null)}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 text-sm border border-slate-300 rounded-md resize-none placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
